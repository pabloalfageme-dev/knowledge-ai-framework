...
- [x] T008 Create `src/core/database.py`: async SQLAlchemy engine (`create_async_engine`), `AsyncSessionLocal` factory, `get_db` FastAPI dependency, and `set_rls_context(session, org_id)` helper that executes `SET LOCAL app.current_org_id = :org_id`; kn_admin support implemented via `set_admin_context(session)` (Option B: `SET LOCAL ROLE kn_admin`) and `set_app_context(session)` (`SET LOCAL ROLE kn_app`) pair — `get_current_user`, `authenticate_user`, and `refresh_tokens` escalate to kn_admin for cross-org lookups then revert via set_app_context; T038–T040 US5 service methods must call set_admin_context for all cross-org operations
...
- [x] T016 Create `tests/auth/integration/conftest.py`: async pytest fixtures — `test_engine` (creates all tables against test PostgreSQL, creates kn_app + kn_admin roles, enables RLS + FORCE RLS, creates tenant_isolation policy, GRANTs table privileges to both roles), `db_session` (issues `SET ROLE kn_app` connection-level so tests run under RLS-enforced role — makes T041 meaningful; patches commit→flush; rolls back after each test), `client` (httpx `AsyncClient` bound to the FastAPI app with get_db overridden to yield the same session); helper fixtures: `make_org`, `make_user` (calls set_rls_context), `make_admin` (wraps make_user), `make_super_admin` (calls set_admin_context for BYPASSRLS insert); teardown does RESET ROLE before DDL so superuser drops succeed; reads `DATABASE_URL` from environment
...
- [x] T026 [US1] Create `src/auth/dependencies.py`: `get_current_user(token=Depends(oauth2_scheme), db=Depends(get_db)) -> User`: decode JWT via `decode_access_token`; check in-process TTL cache keyed by JTI (TTL = `REVOCATION_CACHE_TTL_SECONDS`, default 30 s); on cache miss: query users LEFT JOIN organizations (LEFT JOIN required — super_admin has `organization_id=None` and must not be excluded by an INNER JOIN); assert `u.status == 'active'`; assert `o.status == 'active'` ONLY IF `u.organization_id IS NOT NULL` (super_admin has no org to check); assert `u.token_version == token.tv`; check `revoked_tokens WHERE jti = token.jti` (logout detection); cache positive result; call `set_rls_context(db, u.organization_id)`; return `User`; `require_role(*roles)` → dependency factory wrapping `get_current_user`, raises 403 if `current_user.role not in roles`
...
- [x] T041 Create `tests/auth/integration/test_multitenancy.py` — SC-004 cross-tenant suite: create Org A and Org B, each with admin and regular user; using Org A admin token assert: `GET /users` returns only Org A users (Org B users absent); `PATCH /users/{org_b_user_id}` returns 404; `POST /users` with Org A token creates user in Org A only; `GET /audit` returns only Org A entries; `GET /users/{org_b_user_id}/unlock` returns 404; verify all protected endpoints return 403 or 404 for cross-tenant access 100% of the time; also verify: deactivated org token rejected within one REVOCATION_CACHE_TTL window; NOTE: db_session now issues `SET ROLE kn_app` (connection-level) so these tests verify DB-layer RLS enforcement, not just application WHERE clauses — a bug that removes the WHERE clause would still be caught by RLS
- [x] T042 [P] Create `tests/auth/unit/test_service.py` — unit tests for service business logic with DB `AsyncSession` mocked at boundary: `authenticate_user` increments `failed_login_count` on wrong password; 5th failure sets `locked_until` and writes ACCOUNT_LOCKED audit entry alongside LOGIN_FAILURE; correct password while `locked_until > now()` returns 401; `refresh_tokens` with `used_at IS NOT NULL` revokes entire family (verify UPDATE covers all family members with same `family_id`); `logout` inserts JTI into `revoked_tokens`; `write_audit_entry` failure (mock INSERT raising exception) does NOT propagate — primary auth operation still completes; `update_user` with `role` change increments `token_version`
- [x] T043 [P] Create `tests/auth/unit/test_dependencies.py` — unit tests for `get_current_user` with session mocked: valid JWT + active user + matching `token_version` → returns `User`; expired JWT → raises 401; `token_version` mismatch → raises 401; `user.status='inactive'` → raises 401; `org.status='inactive'` → raises 401; JTI present in `revoked_tokens` → raises 401; second call within TTL with same JTI → DB not queried again (verify mock call count = 1); `require_role("admin")` with `super_admin` user → raises 403; **super_admin case**: user with `organization_id=None` (LEFT JOIN returns `org_status=None`) → authentication succeeds, org status check is skipped
- [x] T044 [P] Create `src/auth/README.md` per Constitution §VIII DoD: module purpose; inputs (HTTP requests, JWT tokens) and outputs (DB records, token pairs); env vars reference table (all vars with types, defaults, and description); Docker usage example; `curl` quickstart for setup → login → refresh → logout flow; how to include `auth_router` and `get_current_user` dependency in another FastAPI module
- [x] T045 Run `quickstart.md` end-to-end validation: `docker compose up -d db && alembic upgrade head && uvicorn src.main:app --reload`; execute Scenarios 1–6 from `quickstart.md/`; verify each `curl` command returns the documented expected output; record any discrepancies as follow-up bugs
- [x] T046 Run `pytest --cov=src/auth --cov-report=term-missing`; fix coverage gaps until ≥80% line coverage is achieved for all modules in `src/auth/` (Constitution §VIII); add targeted unit tests for uncovered branches (typical gaps: error paths in service layer, edge cases in refresh rotation, RLS context setting)
